...
#                 # User granted permission - show profile info
...
#                 # User denied permission - ask for manual entry
#                 self.state["awaiting_profile_permission"] = False
#                 response = """I understand. To assist you, could you please provide:\n1. Your full name\n2. Your city\n3. Your monthly income\n4. Desired loan amount"""
...
#                 return response
#             else:
#                 response = "Please respond with 'Yes' to allow me to access your profile, or 'No' to enter details manually."
#                 self.conversation_history.append({
#                     "role": "assistant",
#                     "content": response
#                 })
...
#             if customer_data:
#                 # Store temporarily and ask for permission
...
class MasterAgent:
    ...
    def __init__(self, api_key: str):
        ...
        self.state = {
            ...
            "final_decision": None,
            "waiting_for_manual_upload": False,
            "user_personality": "friendly" # PACA
        }
    ...
    def _get_personality_prompt(self, personality: str) -> str:
        """Get system prompt instructions for a given personality"""
        prompts = {
            "casual": """
...
May I have your phone number to get started?"""
    ...
    def _handle_new_customer_onboarding(self, user_message: str) -> str:
        """Handle new customer data collection (Placeholder)"""
...
Is this information correct? (Yes/No)"""
    ...
    def _handle_verification(self, user_message: str) -> str:
        """Handle KYC detail confirmation"""
        user_lower = user_message.lower() 
...
Thank you for considering Tata Capital. Have a great day."""
        ...
        if self._is_affirmative(user_lower):
            ...
            self.state["stage"] = "completed"
            self.state["final_decision"] = "REJECTED_KYC"
            response = "I see. To update your details, please contact our support team at 1800-123-4567. For now, I'll have to end this application. Thank you."
            self.conversation_history.append({"role": "assistant", "content": response})
            return response
...
Thank you for exploring all options with us!"""
        ...
        if self._is_affirmative(user_lower) or "option 1" in user_lower:
            self.state["loan_request"]["amount"] = self.state["customer_data"]["pre_approved_limit"]
            self.state["loan_request"]["purpose"] = self.state["loan_request"].get("purpose", "personal")
...
- Learn more about secured loans? (Say "secured loan")"""
            ...
            self.state["awaiting_secured_loan_decision"] = True 
            response = f"""Excellent! Let me explain our secured loan options.
🏠 **Secured Loan Against Property**
- Loan Amount: Up to ₹50 lakhs
- Interest Rate: 8.5% - 10.5%
- Requirement: Property documents (e.g., '{self.state["customer_data"]["collateral"]}')
🚗 **Secured Loan Against Vehicle**
- Loan Amount: Up to 80% of vehicle value
- Interest Rate: 9.5% - 11.5%
- Requirement: Vehicle RC (e.g., '{self.state["customer_data"]["collateral"]}')
...
def main():
    ...
    # --- START OF FIX ---
    # This flag tracks if we've shown the "Conversation completed" message
        ...
        while True:
            try:
                ...
                if user_input.lower() == 'restart':
                    ...
                    master = MasterAgent(api_key)
                    master.start_conversation()
                    final_prompt_shown = False # --- FIX: Reset the flag on restart
                    continue
                ...
                if master.state["stage"] == "completed":
                    ...
                    final_prompt_shown = True # Also set here to prevent re-prompting
                    continue
                
                # --- START OF FIX ---
                # Check all conditions *including* the new flag
                if master.state["stage"] in ["approval", "rejection"] and master.state["final_decision"] and not final_prompt_shown:
                    final_prompt_shown = True # Set the flag so this block doesn't run again
